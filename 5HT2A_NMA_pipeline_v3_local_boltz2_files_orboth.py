...
# CONFIGURATION - SET UP IN THIS FILE TO PULL FROM LOCAL BOLTZ2 FILES

# Output directory for all results
OUTPUT_DIR = "5HT2A_NMA_results_boltz2"
...
# Structure Panel
# Each entry: {
#   'pdb':        PDB ID (used for fetching from RCSB and for labeling),
#   'chain':      receptor chain ID, or 'auto' to pick longest chain,
#   'ligand':     ligand name (for labels),
#   'category':   pharmacological category,
#   'chemotype':  chemical class,
#   'color':      color for plots,
#   'local_path': (OPTIONAL) path to a local PDB/CIF file. If provided,
#                 the script reads from this file instead of fetching from
#                 RCSB. Use this for Boltz2/AlphaFold/docked structures.
#                 The 'pdb' field is still used as a label/identifier.
# }
#
...
# BOLTZ2 LOCAL STRUCTURES (uncomment and edit to use instead of / alongside
# the cryo-EM panel above)

# To run the Boltz2 comparison:
#   1. Generate your co-folded structures and place them in a folder
#   2. Uncomment the block below and update the paths
#   3. Either:
#      (a) Replace STRUCTURES with BOLTZ2_STRUCTURES to run Boltz2 only
#      (b) Set STRUCTURES = STRUCTURES + BOLTZ2_STRUCTURES to run both
#      (c) Keep them separate and run the pipeline twice with different
#          OUTPUT_DIR names for a clean side-by-side comparison
#
BOLTZ2_STRUCTURES = [
    {
        'pdb': 'B2_5HT', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\serotonin_5ht2a.cif',
        'ligand': 'Serotonin (Boltz2)', 'category': 'Full Agonist',
        'chemotype': 'Tryptamine', 'color': '#e74c3c'
    },
    {
        'pdb': 'B2_PSI', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\psilocin_5ht2a.cif',
        'ligand': 'Psilocin (Boltz2)', 'category': 'Full Agonist (Psychedelic)',
        'chemotype': 'Tryptamine', 'color': '#c0392b'
    },
    {
        'pdb': 'B2_DMT', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\dmt_5ht2a.cif',
        'ligand': 'DMT (Boltz2)', 'category': 'Full Agonist (Psychedelic)',
        'chemotype': 'Tryptamine', 'color': '#e67e22'
    },
    {
        'pdb': 'B2_LSD', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\lsd_5ht2a.cif',
        'ligand': 'LSD (Boltz2)', 'category': 'Full Agonist (Psychedelic)',
        'chemotype': 'Ergoline', 'color': '#9b59b6'
    },
    {
        'pdb': 'B2_BOL', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\bol148_5ht2a.cif',
        'ligand': 'BOL-148 (Boltz2)', 'category': 'Partial Agonist (Non-hallucinogenic)',
        'chemotype': 'Ergoline', 'color': '#8e44ad'
    },
    {
        'pdb': 'B2_LIS', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\lisuride_5ht2a.cif',
        'ligand': 'Lisuride (Boltz2)', 'category': 'Agonist (Non-hallucinogenic)',
        'chemotype': 'Ergoline', 'color': '#6c3483'
    },
    {
        'pdb': 'B2_NBOH', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\25cn_nboh_5ht2a.cif',
        'ligand': '25CN-NBOH (Boltz2)', 'category': 'Full Agonist (Psychedelic)',
        'chemotype': 'Phenethylamine', 'color': '#27ae60'
    },
    {
        'pdb': 'B2_MES', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\mescaline_5ht2a.cif',
        'ligand': 'Mescaline (Boltz2)', 'category': 'Full Agonist (Psychedelic)',
        'chemotype': 'Phenethylamine', 'color': '#2ecc71'
    },
    {
        'pdb': 'B2_RS', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\rs130180_5ht2a.cif',
        'ligand': 'RS130-180 (Boltz2)', 'category': 'βArr-biased Agonist',
        'chemotype': 'Phenethylamine', 'color': '#1abc9c'
    },
    {
        'pdb': 'B2_RIS', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\risperidone_5ht2a.cif',
        'ligand': 'Risperidone (Boltz2)', 'category': 'Inactive',
        'chemotype': 'Antipsychotic', 'color': '#2c3e50'
    },
    {
        'pdb': 'B2_MET', 'chain': 'auto',
        'local_path': r'C:\Users\Ben\boltz2_outputs\methiothepin_5ht2a.cif',
        'ligand': 'Methiothepin (Boltz2)', 'category': 'Inverse Agonist',
        'chemotype': 'Inverse Agonist', 'color': '#7f8c8d'
    },
...
#
# # ── To run Boltz2 only: ──
STRUCTURES = BOLTZ2_STRUCTURES
OUTPUT_DIR = "5HT2A_NMA_boltz2"
REFERENCE_PDB = "B2_5HT"  # Use serotonin-Boltz2 as reference
#
# # ── To run both combined: ──
# # STRUCTURES = STRUCTURES + BOLTZ2_STRUCTURES
...
def fetch_and_prepare(entry):
    ...
    Handles:
      - Local PDB/CIF files (via 'local_path' in entry dict)
      - PDB-format files from RCSB (older structures like 6A93, 6WH4, 6WHA)
      - mmCIF-only files from RCSB (newer cryo-EM like 9ARX, 9AS7, etc.)
      - Automatic receptor chain detection (picks longest protein chain)
    """
    pdb_id = entry['pdb']
    chain_id = entry.get('chain', 'auto')
    local_path = entry.get('local_path', None)
    label = f"{entry['ligand']} ({pdb_id})"

    # ── Step 1: Parse the structure ────────────────────────────────────

    # Case A: Local file provided (Boltz2, AlphaFold, docked structures)
    if local_path is not None:
        print(f"  Loading {pdb_id} from {local_path}...", end=" ")
        ...
        try:
            ...
            if ext in ('.cif', '.mmcif'):
                structure = parseMMCIF(local_path)
            else:
                structure = parsePDB(local_path)
            if structure is not None and structure.numAtoms() == 0:
                structure = None
        except Exception as e:
            print(f"ERROR: Could not parse {local_path}!")
            print(f"       {e}")
            return None

    # Case B: Fetch from RCSB
    else:
    ...
    if best_chain is None or best_count < 150:
        # For single-chain local files (common with Boltz2), there may
        # be only one chain or no chain assignment — take all Cα atoms
        if len(all_ca) >= 150:
            print(f"OK (all atoms, {len(all_ca)} Cα, "
                  f"res {all_ca.getResnums()[0]}-{all_ca.getResnums()[-1]}) "
                  f"[single chain / no chain IDs]")
            out_path = os.path.join(OUTPUT_DIR, 'prepared_structures',
                                    f'{pdb_id}_all_CA.pdb')
            writePDB(out_path, all_ca)
            return all_ca
        else:
